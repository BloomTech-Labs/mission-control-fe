...
const UserModal = props => {
  ...
  const handleChange = e => {
    ...
    if (e.target.value !== "Select Your Role") {
      ...
      setSelected({ ...selected, roleId: e.target.value, role: roleName });
      setErr("");
  ...
  return (
    <div>
      <Popup
        trigger={
          <button className={`button`}>Update {user.firstName}'s Role</button>
        }
        modal
      >
        {close => (
          <div className="modal">
            ...
            <div className="header">
              {" "}
              Update {`${user.firstName} ${user.lastName}'s`} Role{" "}
            </div>
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div className="actions">
                  ...
                  {formUsed && !err ? (
                    <Popup
                      trigger={
                        <button className={`button  confirm-btn`} type="button">
                          Confirm Changes
                        </button>
                      }
                      modal
                    >
                      <p className="confirmation-message">
                        Are you sure you want to update {user.firstName}{" "}
                        {user.lastName}'s Role To{" "}
                        {selected.role.charAt(0).toUpperCase() +
                          selected.role.substring(1)}
                      </p>
                      <button
                        type="submit"
