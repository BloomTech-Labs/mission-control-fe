            ...
            props.project.people.map((el, i) => (
              <Card className={classes.card} key={i}>
                  <CardContent className={classes.content}>
                    ...
                    <p className="admin-project-more-team-slack">{<FaSlack style={{marginRight: "0.5rem"}}/>} {el.person.slackId}</p>
                    <p className="admin-project-more-team-timezone">{el.person.timezone}</p>
                    {el.person.program.toLowerCase() === "web" && <p className="product-program-avatar program-web">{el.person.program}</p>}
                    {el.person.program.toLowerCase() === "ds" && <p className="product-program-avatar program-ds">{el.person.program}</p>}
                    {el.person.program.toLowerCase() === "ux/ui" && <p className="product-program-avatar program-ux">{el.person.program}</p>}
                  </CardContent>
              </Card>
            ))
          ) : (
